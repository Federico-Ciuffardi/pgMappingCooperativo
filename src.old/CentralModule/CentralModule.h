...
class CentralModule
{
  private:
    ...
    std::pair < std::list<int> , std::vector< std::list<int> > >  kmeans(int k, std::list<int> puntos, std::vector< cv::Point2f > centros, float dist_lim);
    std::vector< std::list< int > > asignacionKmean(int k, std::list< int > puntos, std::vector< cv::Point2f > centros);
    std::vector< cv::Point2f > actualizacionKmean(std::vector< std::list<int> > puntos_de_centros, 	int cant_centros);
    bool finalizarPorErrorKmean(std::vector< cv::Point2f > centros_viejos, std::vector< cv::Point2f > centros_nuevos, float dist_lim);
