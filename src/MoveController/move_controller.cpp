...
void pointListCallback(const tscf_exploration::goalList& msg){
	if (msg.listaGoals.size() != 0){
		path_saved = msg;
	  //ROS_INFO("MOVE CONTROLLER :: Saving new path. Long -> %lu", msg.listaGoals.size());
		pathflag = 1;
	}
}
...
int main(int argc, char** argv){
  ...
  while(ros::ok()){
    ...
    case 1:{ // AVANZANDO A UN PUNTO
            //ROS_INFO("Sending new objetive %f, %f, %f,", path.listaGoals[path_step].x, path.listaGoals[path_step].y, path.listaGoals[path_step].z);
    ...
    case 2:{
        ...
        ////ROS_INFO("Distance ---> %f, X ---> %f, Y ---> %f", dist_to_target, path.listaGoals[path_step].x, path.listaGoals[path_step].y);
      break;}
