...
class Robot
{
  ...
  private:
    ...
    std::map< int, std::vector<int> > crearOleadas(nav_msgs::OccupancyGrid msg, int fin, std::list<int> centros_def, nav_msgs::OccupancyGrid& p);
    tscf_exploration::frontierReport consultarCostosInfo(std::map< int, std::vector<int> > oleadas, int posicionActual, nav_msgs::OccupancyGrid& p);
  ...
  public:
    ...
    tscf_exploration::goalList getPathToObjetive(int centro, std::vector<int> obstaculos, nav_msgs::OccupancyGrid &p);
    void ajustarParedes(int centro, nav_msgs::OccupancyGrid& p, std::vector<int> obstaculos);
    std::list<int> caminoAfrontera(std::vector<int> oleada, int obj, int start, nav_msgs::OccupancyGrid& p);
    tscf_exploration::goalList getGoalPath(std::list<int> list_camino, nav_msgs::OccupancyGrid& p);
    std::map< int, std::list<int> > obtenerCaminos(int& camino_mas_cercano, std::map< int, std::vector<int> > oleadas, int posicionActual, std::list<int> centros_def, nav_msgs::OccupancyGrid& p);
