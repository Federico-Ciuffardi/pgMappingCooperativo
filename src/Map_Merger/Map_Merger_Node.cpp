...
void handlePose(const geometry_msgs::PoseStamped::ConstPtr& msg, std::string name){
  ROS_DEBUG(" Obtengo nueva pose de  %s", name.c_str());
	if (!FIN){
    map_merger.updatePose(msg, name);
  }
  ROS_DEBUG(" Nueva pose de %s agregada", name.c_str());
}
...
int main(int argc, char* argv[]){
  ...
  for(ros::master::V_TopicInfo::const_iterator it_topic = topic_infos.begin(); it_topic != topic_infos.end(); ++it_topic) {
    ...
    if(published_topic.name.find("/pose") != std::string::npos){
      ...
      pose_sub[nombre] = n.subscribe<geometry_msgs::PoseStamped>(published_topic.name, 1, boost::bind(&handlePose, _1, nombre));
      std::string map_topic = "/" + nombre + "/map";
      map_initialization[nombre] = false;
      map_sub[nombre] = n.subscribe<nav_msgs::OccupancyGrid>(map_topic, 1, boost::bind(&handleNewMap, _1, nombre));
    }
