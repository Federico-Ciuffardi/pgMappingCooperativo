...
bool MapMerger::sonIgualesCeldas(int ind) {
  ...
  while ((ret) && (iterator != maps_by_robots.end())) {
    ret = ((ret && (actual == iterator->second.data[ind])) ||
           (ret && (-1 == iterator->second.data[ind])));
    iterator++;
  }
  return ret;
}
...
int MapMerger::getIndicePosicionActual(float x_ahora, float y_ahora) {
  return (MapMerger::MapMerger::indice_origen +
...
bool MapMerger::isAnyRobotCloser(float dist, int ind, std::string name) {
  ...
  while ((!hay_mas_cerca) && (it != MapMerger::positions.end())) {
    if (((it->first).compare(name) != 0) && (maps_by_robots[it->first].data[ind] != -1)) {
      int posicionAlguno =
          getIndicePosicionActual((it->second).pose.position.x, (it->second).pose.position.y);
...
nav_msgs::OccupancyGrid MapMerger::updateMap(const nav_msgs::OccupancyGridConstPtr& msg,
  ...
  if (!init) {
    ...
    int posicionActual =
        getIndicePosicionActual(positions[name].pose.position.x, positions[name].pose.position.y);

    for (int i = -range; i < (range + 1); i++) {
      for (int j = posicionActual - range * MapMerger::width;
           j < posicionActual + range * MapMerger::width; j += MapMerger::width) {
        int ind = i + j;
        if (msg->data[ind] != -1) {
