...
void CentralModule::saveBid(const tscf_exploration::frontierReportConstPtr& msg, std::string name){
  if (CentralModule::estado == 2){
    ...
    //ROS_INFO(" CENTRAL MODULE :: Guardo Informe del robot %s con nombre %s y cantidad de %d centros", msg->idRobot.c_str(), name.c_str(),msg->cant_centros);
    for (int i = 0; i < msg->cant_centros; i++){
      CentralModule::cost_saved[msg->idRobot][msg->infoCentros[i].centro] = msg->infoCentros[i].cost;
			for (itm=info_gain.begin(); itm!=info_gain.end(); ++itm)
      		if (itm->first != -1){
						CentralModule::info_gain_saved[msg->idRobot][itm->first] = itm->second;
					}
      //ROS_INFO(" -------------------- centro %d ->, costo %d", msg->infoCentros[i].centro, msg->infoCentros[i].cost);
    }
...
bool CentralModule::checkMap(std::map< std::string, bool > mymap){
  ...
  while((ret) && (it!=mymap.end())){
    ret = ret && it->second;
    // ROS_INFO(" CENTRAL MODULE :: ------%s esta %d", it->first.c_str(), it->second);
    ++it;
  }
...
void CentralModule::updateInfoGain(tscf_exploration::asignacionCelda info){
  ...
  for (std::map< std::string,  std::map< int, std::set<int> > >::iterator it = CentralModule::info_gain_saved.begin(); it != CentralModule::info_gain_saved.end(); it++){
    it->second.erase(info.objetivo);
    for (std::map< int, std::set<int> >::iterator it3 = it->second.begin(); it3 != it->second.end(); it3++){
      if (distacia2Puntos(map_points[it3->first], map_points[info.objetivo]) < 12){
        setDifference(it3->second, s);
      }
    }
  }
  for (std::map< std::string,  std::map< int, int > >::iterator it2 = CentralModule::cost_saved.begin(); it2 != CentralModule::cost_saved.end(); it2++){
...
tscf_exploration::asignacionCelda CentralModule::getMaxUtility(){
  ...
  for(it = CentralModule::info_gain_saved.begin(); it != CentralModule::info_gain_saved.end(); it++){
    ...
    for(it_set = it->second.begin();it_set != it->second.end(); it_set++){
      ...
      int cost_celda = CentralModule::cost_saved[it->first][it_set->first];
			...
			//ROS_INFO("CENTRAL MODULE :: robot %s, InfoG %d, Costo %d, Best info %f, best costo %d, candidato %d, actual %d", (it->first).c_str(), info_gain_celda,cost_celda, best_utility, best_cost_celda, dos, info.objetivo );
      cont ++;
      float utility_aux = calcularUtilidad(info_gain_celda, cost_celda);
      if (((utility_aux > best_utility) && (!it->first.compare("") == 0) && (!CentralModule::asignations[info.idRobot])) || ((utility_aux == best_utility) && (best_cost_celda > cost_celda) && (!it->first.compare("") == 0) && (!CentralModule::asignations[info.idRobot]))){
  ...
  if (info.objetivo != -1){
    //ROS_INFO("CENTRAL MODULE :: Candidato Asignado  %s va a %d con COSTO: %d, INFO_G: %d, UTILIDAD %f", (info.idRobot ).c_str(), info.objetivo, best_cost_celda,best_info_gain_celda, best_utility);
  }
  return info;
}
...
std::vector< std::list< int > > CentralModule::asignacionKmean(int k, std::list< int > puntos, std::vector< cv::Point2f > centros){
...
std::vector< cv::Point2f > CentralModule::actualizacionKmean(std::vector< std::list<int> > puntos_de_centros, 	int cant_centros){
	...
	for (int i = 0; i < cant_centros; i++){
		...
		for (it_puntos = puntos_de_centros[i].begin(); it_puntos != puntos_de_centros[i].end(); it_puntos++){
...
bool CentralModule::finalizarPorErrorKmean(std::vector< cv::Point2f > centros_viejos, std::vector< cv::Point2f > centros_nuevos, float dist_lim){
...
/*Funcion que me lleva los centros obtenidos por kmeans a los puntos de mi frontera mas cercanos*/
std::list<int> CentralModule::nearestPoint(std::vector< cv::Point2f > centros_nuevos, std::list<int> puntos){
	std::list<int> points;
...
std::pair < std::list<int> , std::vector< std::list<int> > >  CentralModule::kmeans(int k, std::list<int> puntos, std::vector< cv::Point2f > centros, float dist_lim){
	...
	}while((i <= 100) && (!CentralModule::finalizarPorErrorKmean(centros_viejos, centros_nuevos, dist_lim)));
	ROS_DEBUG(" Ciclos realizados %d", i);

	std::pair < std::list<int> , std::vector< std::list<int> > > retorno(CentralModule::nearestPoint(centros_nuevos, puntos), puntos_de_centros);
...
bool CentralModule::esVecino(int celda, int vecino){
	...
	return ( (vecino == celda-1-map_merged.info.width) || (vecino == celda-map_merged.info.width) || (vecino == celda+1-map_merged.info.width)
	|| (vecino == celda-1) || (vecino == celda+1) || (vecino == celda-1+map_merged.info.width)
	|| (vecino == celda+map_merged.info.width)	|| (vecino == celda+1+map_merged.info.width) );

}
/*dada una celda y una lista de celdas, me dice si la celda es vecina de alguna de las celdas de la lista.*/
...
float CentralModule::distanciaArecta(int inicio, int fin, int punto){
	float dist = abs( ( (map_points[inicio].y - map_points[fin].y) / (map_points[inicio].x - map_points[fin].x) )*map_points[punto].x - map_points[punto].y + (map_points[inicio].y - ( (map_points[inicio].y - map_points[fin].y) / (map_points[inicio].x - map_points[fin].x) )*map_points[inicio].x ) );
	dist = dist / sqrt(pow(( (map_points[inicio].y - map_points[fin].y) / (map_points[inicio].x - map_points[fin].x) ),2) + 1);
	return dist;
}
      ...
      for (int i = 0; i < 9; i++){
        ...
        if (visitadas.find(pos_vecino) == visitadas.end()){
          ...
          if (map_merged.data[pos_vecino] == 100){
            ...
            int key = -1;
            std::map< int, std::pair < std::set<int> , std::set<int> > >::iterator it = obstaculos.begin();
            ...
            if (key != -1){
              ...
              while (puedo && (it_obst != obstaculos[key].first.end())){
                puedo = ((map_merged.data[pos_vecino] == -1) && ( distacia2Puntos(map_points[pos_vecino], map_points[celda]) <= sensor_range )) && ( CentralModule::distanciaArecta(celda, pos_vecino, *it_obst) > dist_info_gain_obst );
                it_obst++;
              }
              if (puedo){
              ...
              if (key != -1){
                ...
                if ((map_merged.data[pos_vecino] == -1) && ( distacia2Puntos(map_points[pos_vecino], map_points[celda]) <= sensor_range )) {
                  info_gain.push_back(pos_vecino);
                  nivel_siguiente.insert(pos_vecino);
                }
              }
...
std::vector<int> CentralModule::aplicarKmeans(std::set<int> frontera){
	...
	for (int i = 1; i <= cantidad_fronteras; i++){
		...
		while (!frontera_completa){
			...
			centros_nuevo.clear();
			puntos_de_centros.clear();
			// Inicializo los cnetros con un elemento mas, de la misma forma que las inicializaciones anteriores
			...
			// Aplico kmeans
			std::pair < std::list<int> , std::vector< std::list<int> > > retornoKmean = CentralModule::kmeans(k, clusters[i], centros, 0.1);
